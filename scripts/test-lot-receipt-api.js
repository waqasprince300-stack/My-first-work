...
connectDB()
  .then(async () => {
    ...
    console.log("Simulating admin fetch for", {
      lotIdStr,
    ...
    console.log(
      "Exact match:",
      Boolean(row),
      row?.receipt ? `receipt len ${row.receipt.length}` : "no receipt",
    );
    ...
    const lot = await GhausiaLot.findOne({ _id: lotIdStr, userId })
      ...
      .lean();
    console.log("Lot lookup:", {
      found: Boolean(lot),
      biz: lot ? String(lot.businessOwnerId) : null,
    });
