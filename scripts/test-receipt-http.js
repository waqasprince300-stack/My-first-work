...
connectDB()
  .then(async () => {
    const admins = await User.find({ role: 'admin', status: 'approved' }).limit(5).lean();
    console.log('Admins:', admins.map((u) => String(u._id)));
    ...
    for (const user of admins) {
      ...
      console.log('\nTesting user', user.email, 'lot', lotId, 'biz', biz, 'ownerActive', Boolean(owner));
      ...
      console.log('LOCAL', res.status, 'receiptLen', body?.receipt?.length || 0, body?.message || '');
      ...
      console.log('PROD', res2.status, 'receiptLen', body2?.receipt?.length || 0, body2?.message || '');
      break;
    }
