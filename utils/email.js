...
const ensureMailConfig = () => {
  ...
  const placeholders = requiredMailConfig.filter((key) =>
    placeholderValues.has(getEnv(key)),
  );

  if (placeholders.length > 0) {
    throw new Error(
      `Replace placeholder email configuration: ${placeholders.join(", ")}`,
    );
  }
};
...
const sendLotNotificationEmail = async ({
  to,
  name,
  subject,
  body,
