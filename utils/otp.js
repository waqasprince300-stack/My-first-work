...
const deliverOtp = async ({ user, channel, code, purpose }) => {
  ...
  return {
    ...
    note: res.delivered ? '' : 'SMS gateway is in log mode — no real message was sent.',
  };
};
 ...
 */
const createAndSendOtp = async ({ user, purpose, preferredChannel, deviceId }) => {
  const channel = resolveChannel(user, preferredChannel);
  if (!channel) {
    const err = new Error('No email or phone number is on file to send a verification code.');
...
const checkOtp = (otp, code) => {
  ...
  if (otp.attempts >= OTP_MAX_ATTEMPTS) return { ok: false, reason: 'too_many' };
