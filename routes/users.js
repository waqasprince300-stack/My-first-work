...
const findAdminParty = async (adminId, partyId, businessOwnerId) => {
  ...
  const partyBiz =
    party.businessOwnerId != null ? String(party.businessOwnerId) : "";
  const reqBiz =
    businessOwnerId != null && String(businessOwnerId).trim() !== ""
      ? String(businessOwnerId).trim()
      : "";
...
router.get("/", async (req, res) => {
  try {
    if (!isTenantAdmin(req.user)) {
      return res
        .status(403)
        .json({ message: "Business admin access required" });
    }
...
router.get("/pending-parties", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error fetching pending parties",
        error: error.message,
      });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (user.role !== "party" || user.status !== "pending") {
        return res
          .status(400)
          .json({ message: "This action only applies to pending party users" });
      }
      ...
      if (String(user.pendingForAdminId) !== String(req.user._id)) {
        return res
          ...
          .json({
            message:
              "You can only approve users who requested your organization",
          });
      }

      const linkedParty = await findAdminParty(
        req.user._id,
      ...
      if (!linkedParty) {
        return res
          .status(400)
          .json({ message: "Select a valid party for this user" });
      }
      ...
      user.businessOwnerId =
        req.businessOwnerId != null ? String(req.businessOwnerId) : "";
...
router.patch("/:id/reject", async (req, res) => {
  try {
    ...
    if (user.status !== "pending") {
      return res
        .status(400)
        .json({ message: "Only pending accounts can be rejected here" });
    }
...
router.patch("/:id/disable", async (req, res) => {
  try {
    ...
    if (
      user.role !== "party" ||
      String(user.ownerId) !== String(req.user._id) ||
      user.status !== "approved"
    ) {
      return res
        .status(400)
        .json({
          message:
            "You can only disable approved party users in your organization",
        });
    }
...
router.patch("/:id/enable", async (req, res) => {
  try {
    ...
    if (String(user._id) === String(req.user._id)) {
      return res
        .status(400)
        .json({ message: "You cannot change your own account here" });
    }
    ...
    if (
      ...
      return res
        .status(403)
        .json({
          message: "You can only manage party users in your organization",
        });
    }
    ...
    if (user.status !== "disabled") {
      return res
        .status(400)
        .json({ message: "Only disabled party users can be re-enabled" });
    }
    ...
    try {
      ...
      if (!["approved", "disabled"].includes(user.status)) {
        return res
          ...
          .json({
            message:
              "Only approved or disabled party users can have their party changed",
          });
      }
      ...
      user.partyName = linkedParty.name;
      user.businessOwnerId =
        linkedParty.businessOwnerId != null &&
        String(linkedParty.businessOwnerId).trim() !== ""
          ? String(linkedParty.businessOwnerId)
          : req.businessOwnerId != null
            ? String(req.businessOwnerId)
            : user.businessOwnerId;
      await user.save({ validateBeforeSave: false });
      ...
      res
        .status(400)
        .json({ message: "Error changing party", error: error.message });
    }
  },
);
