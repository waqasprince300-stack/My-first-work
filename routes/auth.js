...
const respondWithToken = async (res, statusCode, user, { deviceId, deviceLabel, ...extra } = {}) => {
  if (deviceId) {
    try {
...
router.post('/login', authLimiter, async (req, res) => {
  try {
    ...
    if (user.role !== 'super_admin' && !deviceId) {
      return res.status(400).json({
        message: 'A device identifier is required. Please sign in using the official web app.',
...
router.post('/login/verify-otp', otpLimiter, async (req, res) => {
  try {
    ...
    if (!otp || otp.purpose !== 'login_device') {
      return res.status(400).json({ message: 'This code is invalid or has expired. Request a new one.' });
    }
...
router.post('/login/resend-otp', otpLimiter, async (req, res) => {
  try {
    ...
    if (!prev || prev.purpose !== 'login_device') {
      return res.status(400).json({ message: 'Could not resend the code. Please sign in again.' });
    }

    if ((prev.resendCount || 0) >= OTP_MAX_RESENDS) {
      return res.status(429).json({ message: 'Too many code requests. Please wait a while and sign in again.' });
    }
...
router.post('/password-reset/request', authLimiter, async (req, res) => {
  try {
    ...
    const generic = 'If that account is registered, a verification code has been sent.';
    const user = phone
    ...
    try {
      ...
      if (otpErr.code === 'NO_OTP_DESTINATION') {
        return res.status(400).json({
          message: 'This account has no email or phone on file to send a code to. Contact support.',
...
router.post('/password-reset/verify', otpLimiter, async (req, res) => {
  try {
    ...
    const user = await User.findById(otp.userId).select('+password +knownDevices');
    if (!user) {
...
router.post('/account/upgrade', authenticate, async (req, res) => {
  try {
    const user = await User.findById(req.user._id).select('+password +knownDevices');
    if (!user) {
      return res.status(404).json({ message: 'Account not found' });
    }
    if (user.role !== 'personal_khata') {
      return res.status(400).json({ message: 'Only Personal Khata accounts can be upgraded.' });
    }

    const targetRole = String(req.body.targetRole || '').toLowerCase() === 'admin' ? 'admin' : 'party';
    ...
    if (!user.email) {
      ...
      if (!bodyEmail) {
        return res.status(400).json({
          message: 'Business accounts sign in with email. Add an email address to upgrade.',
          code: 'EMAIL_REQUIRED',
        });
      }
      ...
      const exists = await User.findOne({ email: bodyEmail, _id: { $ne: user._id } });
      if (exists) {
        return res.status(409).json({ message: 'Email is already registered' });
      }
    ...
    if (targetRole === 'admin') {
      const hasSuperAdmin = await User.exists({ role: 'super_admin', status: 'approved' });
      ...
      await user.save({ validateBeforeSave: false });
      return respondWithToken(res, 200, user, {
        message: 'Upgraded to business administrator. You are now signed in as an admin.',
      });
    }

    // Party upgrade — needs an approved business administrator.
    const adminEmail = req.body.adminEmail ? String(req.body.adminEmail).trim().toLowerCase() : '';
    if (!adminEmail) {
      return res.status(400).json({ message: 'Enter your business administrator email.' });
    }
    const adminErr = getRegistrationEmailError(adminEmail);
    if (adminErr) {
      return res.status(400).json({ message: `Business administrator email: ${adminErr}` });
    }
    const targetAdmin = await User.findOne({ email: adminEmail, role: 'admin', status: 'approved' });
    if (!targetAdmin) {
      return res.status(400).json({
