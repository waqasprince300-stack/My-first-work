...
const getAllowedOrigins = () => {
  const corsOrigin = process.env.CORS_ORIGIN || "https://seamandgrace.com";
...
io.use(async (socket, next) => {
  ...
  try {
    ...
    const user = await User.findById(decoded.id)
      .select("role ownerId approvedBy status")
      .lean();
...
app.use(
  "/api/rateCalculations",
  authenticate,
  requireApproved,
  resolveBusinessOwner,
  rateCalculationsRouter,
);
app.use(
  "/api/savedDesigns",
  authenticate,
...
const startServer = async () => {
  ...
  if (isProduction && !explicitPort) {
    if (isPassenger) {
      ...
      console.log(
        "✅ App ready for cPanel Passenger (export-only — no port bind)",
      );
  ...
  server.on("error", (err) => {
    if (err.code === "EADDRINUSE") {
      ...
      console.error(
        "   cPanel: Stop App → wait 15s → Restart App (one instance only).",
      );
    } else {
